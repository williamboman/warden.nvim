...
---@param opts WardenDiagnosticOpts
local function redraw_extmarks(bufnr, trigger_ns, opts)
	---@type Diagnostic[]
	...
	for line, diagnostics in pairs(diagnostic_lines(merged_diagnostics)) do
		...
		vim.api.nvim_buf_set_extmark(bufnr, highest_severity_diagnostic.namespace, line, 0, {
			hl_mode = "combine",
			priority = 0,
			line_hl_group = opts.line_highlight and line_highlight_map[highest_severity_diagnostic.severity] or nil,
...
function M.show(namespace, bufnr, diagnostics, opts)
	...
	if not ns.user_data.warden_ns then
		ns.user_data.warden_ns = create_namespace()
	end
	diagnostics_per_namespace[ns.user_data.warden_ns][bufnr] = prefix_source(diagnostics, ns.user_data.warden_ns)
	redraw_extmarks(bufnr, ns.user_data.warden_ns, opts.warden)
end
...
function M.hide(namespace, bufnr)
	bufnr = get_bufnr(bufnr)
	local ns = vim.diagnostic.get_namespace(namespace)
	if ns.user_data.warden_ns then
		vim.api.nvim_buf_clear_namespace(bufnr, ns.user_data.warden_ns, 0, -1)
		diagnostics_per_namespace[ns.user_data.warden_ns][bufnr] = nil
	end
end
